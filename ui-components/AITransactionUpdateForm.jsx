...
export default function AITransactionUpdateForm(props) {
  const {
    id: idProp,
    aITransaction: aITransactionModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = aITransactionRecord
      ? { ...initialValues, ...aITransactionRecord }
      : initialValues;
    setAIServiceID(cleanValues.AIServiceID);
    setAIModelID(cleanValues.AIModelID);
    setRequestJSON(cleanValues.RequestJSON);
    setRequestDateTime(cleanValues.RequestDateTime);
    setResponseJSON(cleanValues.ResponseJSON);
    setResponseDateTime(cleanValues.ResponseDateTime);
    setErrors({});
  };
  const [aITransactionRecord, setAITransactionRecord] = React.useState(
    aITransactionModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(AITransaction, idProp)
        : aITransactionModelProp;
      setAITransactionRecord(record);
    };
    queryData();
  }, [idProp, aITransactionModelProp]);
  React.useEffect(resetStateValues, [aITransactionRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            AITransaction.copyOf(aITransactionRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "AITransactionUpdateForm")}
      {...rest}
    >
      <TextField
        label="Ai service id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || aITransactionModelProp)}
