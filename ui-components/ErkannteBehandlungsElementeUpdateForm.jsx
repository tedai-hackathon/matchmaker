...
export default function ErkannteBehandlungsElementeUpdateForm(props) {
  const {
    id: idProp,
    erkannteBehandlungsElemente: erkannteBehandlungsElementeModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = erkannteBehandlungsElementeRecord
      ? { ...initialValues, ...erkannteBehandlungsElementeRecord }
      : initialValues;
    setErfassungsZeitpunkt(cleanValues.ErfassungsZeitpunkt);
    setKunde(cleanValues.Kunde);
    setTiere(cleanValues.Tiere);
    setVorbericht(cleanValues.Vorbericht);
    setBefunde(cleanValues.Befunde);
    setDiagnosen(cleanValues.Diagnosen);
    setTherapieMassnahmen(cleanValues.TherapieMassnahmen);
    setVerwendeteTherapieMittel(cleanValues.VerwendeteTherapieMittel);
    setAbgegebeneTherapieMittel(cleanValues.AbgegebeneTherapieMittel);
    setInPegassusErfasst(cleanValues.InPegassusErfasst);
    setErrors({});
  };
  const [
    erkannteBehandlungsElementeRecord,
    setErkannteBehandlungsElementeRecord,
  ] = React.useState(erkannteBehandlungsElementeModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(ErkannteBehandlungsElemente, idProp)
        : erkannteBehandlungsElementeModelProp;
      setErkannteBehandlungsElementeRecord(record);
    };
    queryData();
  }, [idProp, erkannteBehandlungsElementeModelProp]);
  React.useEffect(resetStateValues, [erkannteBehandlungsElementeRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            ErkannteBehandlungsElemente.copyOf(
              erkannteBehandlungsElementeRecord,
              (updated) => {
      ...
      {...getOverrideProps(overrides, "ErkannteBehandlungsElementeUpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || erkannteBehandlungsElementeModelProp)}
