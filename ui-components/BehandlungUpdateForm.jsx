...
export default function BehandlungUpdateForm(props) {
  const {
    id: idProp,
    behandlung: behandlungModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = behandlungRecord
      ? { ...initialValues, ...behandlungRecord }
      : initialValues;
    setErfassungsZeitpunkt(cleanValues.ErfassungsZeitpunkt);
    setInPegassusErfasst(cleanValues.InPegassusErfasst);
    setErrors({});
  };
  const [behandlungRecord, setBehandlungRecord] =
    React.useState(behandlungModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Behandlung, idProp)
        : behandlungModelProp;
      setBehandlungRecord(record);
    };
    queryData();
  }, [idProp, behandlungModelProp]);
  React.useEffect(resetStateValues, [behandlungRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Behandlung.copyOf(behandlungRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "BehandlungUpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || behandlungModelProp)}
