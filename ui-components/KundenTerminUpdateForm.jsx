...
export default function KundenTerminUpdateForm(props) {
  const {
    id: idProp,
    kundenTermin: kundenTerminModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = kundenTerminRecord
      ? { ...initialValues, ...kundenTerminRecord }
      : initialValues;
    setKundenName(cleanValues.KundenName);
    setKundenAnfrage(cleanValues.KundenAnfrage);
    setTerminVreinbarung(cleanValues.TerminVreinbarung);
    setOrtsVereinbarung(cleanValues.OrtsVereinbarung);
    setAertztlicherVorbericht(cleanValues.AertztlicherVorbericht);
    setNotwendigeVorbereitungenIds(cleanValues.NotwendigeVorbereitungenIds);
    setErrors({});
  };
  const [kundenTerminRecord, setKundenTerminRecord] = React.useState(
    kundenTerminModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(KundenTermin, idProp)
        : kundenTerminModelProp;
      setKundenTerminRecord(record);
    };
    queryData();
  }, [idProp, kundenTerminModelProp]);
  React.useEffect(resetStateValues, [kundenTerminRecord]);
  const validations = {
    KundenName: [],
    KundenAnfrage: [],
    TerminVreinbarung: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            KundenTermin.copyOf(kundenTerminRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "KundenTerminUpdateForm")}
      {...rest}
    >
      <TextField
        label="Kunden name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || kundenTerminModelProp)}
